...
#### Servicio `json_app`:
- **Nombre de Contenedor**: Nombre del contenedor configurado como `jsonplaceholder_container`.
- **Build**: Construye la imagen utilizando el contexto actual (`.`) y el archivo `Dockerfile`.
- **Variables de entorno**:
  - `DB_HOST`: Host de la base de datos PostgreSQL, configurado como `postgres`.
  - `DB_DATABASE`: Nombre de la base de datos PostgreSQL, configurado como `jsonplaceholder_data`.
  - `DB_USER`: Usuario de PostgreSQL, configurado como `postgres`.
  - `DB_PASSWORD`: Contraseña del usuario de PostgreSQL, configurada como `1234567890`.
  - `DB_PORT`: Puerto de PostgreSQL, configurado como `5432`.
- **Dependencias**: Depende del servicio `postgres`.
- **Redes**: Se conecta a la red `dem`.
- **Comando**: Ejecuta el script Python `jsonplaceholder.py` al iniciar el contenedor.



#### Redes:
- **Nombre**: Define la red personalizada como `dem`.
- **Driver**: Utiliza el driver de red `bridge`.

#### Volúmenes:
- **Nombre**: Crea el volumen `postgres_data` para almacenar los datos persistentes de PostgreSQL.

Este archivo `docker-compose.yml` facilita la creación y gestión de múltiples contenedores Docker para PostgreSQL, pgAdmin y una aplicación Python, permitiendo la comunicación entre ellos en la red `dem`.
